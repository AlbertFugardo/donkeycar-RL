...
# exe_path = "/mnt/c/users/Usuari/OneDrive/Escritorio/4t/PRIMAVERA/I2RCED/DonkeySimWin/donkey_sim.exe"
# port = 9093
...
for frame_num in range(total_frames):
    ...
    for event in pygame.event.get():
        if (event.type == QUIT or event.type == KEYDOWN) and event.key in [  # pytype: disable=name-error
            ...
            with open("center_line_points", "wb") as fp:   #Pickling
                pickle.dump(center_line_points, fp)
            env.close()
            exit()
    ...
    for _ in range(frame_skip):
        ...
        center_line_points.append(info["pos"])
        if done:
            break
    if render:
        env.render()
